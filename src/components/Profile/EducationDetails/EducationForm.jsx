...
const EducationForm = ({ show, onClose, onSave, initialData = {} }) => {
  const [formData, setFormData] = useState({education: initialData?.education || ''});
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const newErrors = {};
    const requiredFields = ['education', 'university', 'course', 'specialization', 'courseType', 'startYear', 'endYear', 'grading'];
    if (showOtherMarksField) requiredFields.push('marks');
    if (isOtherCourse) requiredFields.push('otherCourse');

    requiredFields.forEach(field => {
      if (!formData[field] && !(field === 'otherCourse' && otherCourse)) {
    ...
    const payload = {
      ...formData,
      otherCourse: isOtherCourse ? otherCourse : '',
    };

    const method = formData.id ? 'put' : 'post';
    const url = formData.id
      ? `http://localhost:8080/api/education/${formData.id}`
  ...
  const handleDelete = () => {
    ...
    if (window.confirm("Are you sure you want to delete this education record?")) {
      axios
        .delete(`http://localhost:8080/api/education/${formData.id}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        })
        .then(() => {
          onSave({ deletedId: formData.id }); // Notify parent
          onClose();
        })
        .catch((err) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setIsOtherCourse(initialData.course === 'other');
      setOtherCourse(initialData.otherCourse || '');
      const shouldShowMarks =
        ['Grade-10', 'Grade-4', 'marks'].includes(initialData.grading);
      setShowOtherMarksField(shouldShowMarks);

      if (shouldShowMarks && marksRef.current) {
        ...
        {formData?.id && (
          <Button
            ...
            onClick={handleDelete}
             style={{
          position: 'absolute',
          right: '3.5rem', // adjust as needed depending on your close button size
        ...
        <Form id="educationForm" onSubmit={handleSubmit}>
          ...
          <Form.Group className='mb-3'>
            ...
            <Creatable
              ...
              onChange={(selectedOption) => {
                const selectedValue = selectedOption?.value || '';
                handleChange({ target: { name: 'course', value: selectedValue } });
